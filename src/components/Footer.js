            ...
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-12">
                    ...
                    <div className="space-y-4 lg:col-span-1">
                        <Link href="/" className="block">
                            <Image
                                src="/black_logo.png"
                                alt="Zugo - Bike & Car Rental India"
                                width={80}
                                height={26}
                    ...
                    <div>
                        ...
                        <ul className="space-y-3 text-sm text-gray-500">
                            ...
                            <li><Link href="/book/bike" className="hover:text-black transition-colors">Book a Bike</Link></li>
                            <li><Link href="/book/car" className="hover:text-black transition-colors">Book a Car</Link></li>
                    ...
                    <div>
                        <h3 className="font-bold text-gray-900 mb-4">Bike Rentals by City</h3>
                        <ul className="space-y-3 text-sm text-gray-500">
                            {Object.entries(cityContent).map(([key, city]) => (
                                <li key={key}>
                                    <Link href={`/bikes-for-rent/${key}`} className="hover:text-black transition-colors">
                                        Bike Rental in {city.name}
                                    </Link>
                                </li>
                            ))}
                        </ul>
                        <h3 className="font-bold text-gray-900 mb-4 mt-6">Rentals by City</h3>
                        <ul className="space-y-3 text-sm text-gray-500">
                            {Object.entries(cityContent).map(([key, city]) => (
                                <li key={key}>
                                    <Link href={`/rentals/${key}`} className="hover:text-black transition-colors">
                ...
                <div className="pt-8 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                    <p className="text-gray-400 text-sm">
                        © {new Date().getFullYear()} Zugo Rentals. All rights reserved. | Bike Rental in Rishikesh, Delhi, Bangalore & Gurugram
                    </p>
